  ...
  CRM.BookingApp.module('AddSubResource', function(AddSubResource, BookingApp, Backbone, Marionette, $, _) {
    ...
    CRM.BookingApp.vent.on("render:price", function(model) {
      ...
      try{
        if(model.attributes.total_price>=0){
          totalText = model.attributes.total_price.toFixed(2);
        }
      }catch(err){}
      ...
      try{
        subtotalText = model.attributes.sub_total.toFixed(2);
      }catch(err){}
    ...
    AddSubResource.AddSubResourceModal = BookingApp.Common.Views.BookingProcessModal.extend({
      ...
      onRender: function(){
        ...
        CRM.api3('Resource', 'get', {'sequential': 1, 'is_unlimited': 1, 'is_deleted': 0, 'is_active': 1},
      ...
      onValidateRulesCreate: function(view, r) {
        $.validator.addMethod("withinValidTime", function(value, element) {
          ...
          if (unlimitedTimeConfig==0){
            ...
            return ((requiredDate>=minDate) && (requiredDate<=maxDate));
          }
            ...
            { context: self,
              success: function(data) {
                ...
                if(configSet.count !== 1){
                  url = CRM.url('civicrm/admin/resource/config_set', {
                    reset: 1,
                    action: 'update',
                    id: resource.id
                  });
                  ...
                  url = CRM.url('civicrm/admin/resource/config_set/config_option', {
                    reset: 1,
      ...
      addSubResource: function(e){
        ...
        var resourceTotalPrice = 0;
        if(this.isNew){
          resourceTotalPrice = parseFloat(currentResourceTotal) + parseFloat(priceEstimate);
          priceCache[subResourceRefId] = parseFloat(priceEstimate);
        }else{
          resourceTotalPrice = parseFloat(currentResourceTotal) + parseFloat(priceEstimate) - priceCache[subResourceRefId];
    ...
    AddSubResource.EditAdhocChargesModal = BookingApp.Common.Views.BookingProcessModal.extend({
      ...
      updateAdhocCharges: function(e){
        ...
        if(CRM.BookingApp.Utils.isPositiveNumber(discountAmount)){
          newTotal = (parseFloat(adhocChargesTotal) + parseFloat(currentTotal)) - parseFloat(discountAmount);
        }else{
          newTotal = (parseFloat(adhocChargesTotal) + parseFloat(currentTotal)) - 0;
        }
